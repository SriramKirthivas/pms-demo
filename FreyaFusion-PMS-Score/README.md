# FreyaFusion-PMS-Score

Score service (pm-score) of the Freya Fusion PMS module. **Scaffold** — implement per the spec.

- **Stack:** Python / FastAPI (Pydantic + SQLAlchemy), containerised for ECS.
- **Database:** owns `pm_score` (via `SERVICE_DB`), derived from `DATABASE_URL`.
...
uvicorn app.main:app --reload --port 8000   # GET /api/health, /api/pm-score/ping
pytest -q
```

Context path: `/api/pm-score`.
